...
function BlogIntro() {
	return (
		<div>
			...
			<br />
			<br />
			This is the corresponding blog for a course in Generic Programming
			and the STL at UT Austin :)
			</p>
			<h3></h3>
			</div>
		</div>
...
function GenericAndSTL(){
	return (
		<div>
			<BlogIntro />
			<h2 style={{textAlign: 'center'}}>{"CS371g Summer 2021: Krithika Ravishankar"}</h2>
			{ listwrite.map (item => <Makebutton title={item.title} path={item.path} date={item.date} desc={item.desc} /> ) }
		</div>
	)
}
export default GenericAndSTL;
